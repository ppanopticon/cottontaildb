 ...
 * @return [CottontailGrpc.SchemaName] for the given [Name.SchemaName].
 */
fun Name.EntityName.protoFrom(): CottontailGrpc.From = CottontailGrpc.From.newBuilder()
    .setScan(CottontailGrpc.Scan.newBuilder().setEntity(this.proto())).build()
...
fun CottontailGrpc.KnnHint.toHint(): KnnPredicateHint? = when (this.hintCase) {
    ...
    CottontailGrpc.KnnHint.HintCase.TYPEINDEXHINT -> KnnPredicateHint.IndexTypeHint(
        IndexType.valueOf(
            this.typeIndexHint.type.toString()
        )
    )
    CottontailGrpc.KnnHint.HintCase.NAMEINDEXHINT -> KnnPredicateHint.IndexNameHint(
        Name.IndexName(
            this.nameIndexHint.name
        ), this.nameIndexHint.parametersMap
    )
    CottontailGrpc.KnnHint.HintCase.PARALLELINDEXHINT -> KnnPredicateHint.ParallelKnnHint(
        this.parallelIndexHint.min,
        this.parallelIndexHint.max
    )
    else -> null
}