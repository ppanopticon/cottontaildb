 ...
 * @version 1.0
 */
internal class UnaryScanningKnn : AbstractScanningKnn() {
    /**
     ...
     */
    fun evaluate(source: Name, id_field: Name, vec_field: Name, query: List<BigDecimal>, k: Int, distance: String) : CottontailKnnTable {
        ...
        val results = when(vecCol.type) {
            is DoubleArrayType -> doubleKnn(tx = tx, id_col = idCol, vec_col = vecCol as ColumnDef<DoubleArray>, k = k, distance = Distance.valueOf(distance), queries = listOf(DoubleArray(query.size) { query[it].toDouble() }), weights = null)
            is FloatArrayType -> floatKnn(tx = tx, id_col = idCol, vec_col = vecCol as ColumnDef<FloatArray>, k = k,  distance = Distance.valueOf(distance), queries = listOf(FloatArray(query.size) { query[it].toFloat() }), weights = null)
            is LongArrayType -> longKnn(tx = tx, id_col = idCol, vec_col = vecCol as ColumnDef<LongArray>, k = k,  distance = Distance.valueOf(distance), queries = listOf(LongArray(query.size) { query[it].toLong() }), weights = null)
            is IntArrayType -> intKnn(tx = tx, id_col = idCol, vec_col = vecCol as ColumnDef<IntArray>, k = k, distance = Distance.valueOf(distance), queries = listOf(IntArray(query.size) { query[it].toInt() }), weights = null)
